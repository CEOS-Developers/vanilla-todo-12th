...
function addList (e) {
    if (e == 'click' || e.keyCode == 13) {
        if (document.getElementById("add_input").value != '') {
            ...
            document.getElementById('wait-list').append(listbox);
...
function move(li) {
    if (li.className == "waiting") {    // 클릭한 것이 대기중인 일이라면 완료됨으로 이동.
        ...
        document.getElementById("complete-list").appendChild(li.parentNode);
    } else {    // 완료된 일이라면 대기중으로 이동.
        li.className = "waiting";
        document.getElementById("wait-list").appendChild(li.parentNode);
    }
...
function checkListNum() {
    document.querySelector(".wait").textContent = `대기중 (${waitingNum.length})`;
    document.querySelector(".complete").textContent = `완료됨 (${completeNum.length})`;
